...
//glsl / sprvi related header files
...
VkResult createShaders(void)
{
	...
	if (bDone == false)
	{
		...
		fprintf(gpFile, "createShaders() - glsl2Spv function Successed for vertex shader\n");
	}
	...
	if (vkResult != VK_SUCCESS)
	{
		fprintf(gpFile, "createShaders() - createShaders function failed for vertex shader %d \n", vkResult);
	}
	else
	{
		fprintf(gpFile, "createShaders() - createShaders function Successed for vertex shader \n");
	}
	...
	if (bDone == false)
	{
		...
		fprintf(gpFile, "createShaders() - glsl2Spv function Successed for fragment shader\n");
	}
...
bool glsl2Spv(const VkShaderStageFlagBits shader_type, const char* pShader, vector<unsigned int>& spv)
{
	...
	//code
	//find the shader language in the form of esh language
	EShLanguage language = findShaderLanguage(shader_type);

	//create TShader TypeShader from above eshLanguage by calling TShader constructor
	...
	//specify rules of shader compilation as sprvi rules and vulkan rules
	EShMessages ruleMessage = (EShMessages)(EShMsgSpvRules | EShMsgVulkanRules);

	//Parse the T shader 
	if (!shader.parse(&resources, 100, false, ruleMessage))
	{
		fprintf(gpFile, "glsl2Spv() - parse() method failed with log %s\n", shader.getInfoLog());
		fprintf(gpFile, "glsl2Spv() - parse() method failed with debug log %s\n", shader.getInfoDebugLog());
		return FALSE;
	}
	else {
		fprintf(gpFile, "glsl2Spv() - parse() method successded\n");
	}
	...
	//link the program
	if (!program.link(ruleMessage))
	{
		fprintf(gpFile, "glsl2Spv() - link() method failed with log %s\n", shader.getInfoLog());
		fprintf(gpFile, "glsl2Spv() - link() method failed with debug log %s\n", shader.getInfoDebugLog());
		return FALSE;
	}
	else {
		fprintf(gpFile, "glsl2Spv() - link() method successded\n");
	}

	//add now send cooked program and sprvi code to glslang library
