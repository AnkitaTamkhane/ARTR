...
void unintialize(void)
{
	...
	if (vertexData_position_index.vkDeviceMemory)
	{
		...
		fprintf(gpFile, "\n unintialize()- vertexData_position_index.vkDeviceMemory destroy successed");
	}
	...
	if (vertexData_position_index.vkBuffer)
	{
		...
		fprintf(gpFile, "\n unintialize()- vertexData_position_index.vkBuffer destroy successed");
	}
...
VkResult createVertexBuffer(void)
{
	...
	if (vkResult != VK_SUCCESS)
	{
		fprintf(gpFile, "createVertexBuffer() - vkMapMemory function failed for Vertex position buffer  %d \n", vkResult);
	}
	else
	{
		fprintf(gpFile, "createVertexBuffer() - vkMapMemory function Successed for vertex position buffer  \n");
	}
	...
	if (vkResult != VK_SUCCESS)
	{
		fprintf(gpFile, "createVertexBuffer() - vkCreateBuffer function failed for position Index Buffer %d \n", vkResult);
	}
	else
	{
		fprintf(gpFile, "createVertexBuffer() - vkCreateBuffer function Successed for position Index Buffer \n");
	}
	...
	if (vkResult != VK_SUCCESS)
	{
		fprintf(gpFile, "createVertexBuffer() - vkAllocateMemory function failed for position index buffer  %d \n", vkResult);
	}
	else
	{
		fprintf(gpFile, "createVertexBuffer() - vkAllocateMemory function Successed for position index buffer  \n");
	}

	vkResult = vkBindBufferMemory(vkDevice, vertexData_position_index.vkBuffer, vertexData_position_index.vkDeviceMemory, 0);
	if (vkResult != VK_SUCCESS)
	{
		fprintf(gpFile, "createVertexBuffer() - vkBindBufferMemory function failed for position index buffer   %d \n", vkResult);
	}
	else
	{
		fprintf(gpFile, "createVertexBuffer() - vkBindBufferMemory function Successed for position index buffer  \n");
	}

	vkResult = vkMapMemory(vkDevice, vertexData_position_index.vkDeviceMemory, 0, vkMemoryAllocateInfo.allocationSize, 0, &data);
	if (vkResult != VK_SUCCESS)
	{
		fprintf(gpFile, "createVertexBuffer() - vkMapMemory function failed for position index buffer  %d \n", vkResult);
	}
	else
	{
		fprintf(gpFile, "createVertexBuffer() - vkMapMemory function Successed for position index buffer  \n");
	}
...
VkResult buildCommandBuffers(void)
{
	...
	for (uint32_t i = 0; i < swapchainImageCount; i++)
	{
		...
		//4th parameter is correspondence to 3 param gl_draw element , 2nd para is correspondence to gl_draw element
		vkCmdBindVertexBuffers(vkCommandBuffer_array[i],0,1,&vertexData_position.vkBuffer, vkDeviceSize_offset_array);

		//bind index buffer
		//2nd parameter is correspondence to 2bd param gl_draw elements
		vkCmdBindIndexBuffer(vkCommandBuffer_array[i],vertexData_position_index.vkBuffer,0,VK_INDEX_TYPE_UINT32); // vk_index_type_uint32 match globally declare triangle_position_indices
		
		//here we should call vulkan drawing function
		unsigned int num_indices = (unsigned)(sizeof(triangle_position_indices) / sizeof(triangle_position_indices[0]));

		vkCmdDrawIndexed(vkCommandBuffer_array[i],num_indices,1,0,0, 1);
